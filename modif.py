...
# # Fungsi untuk interpolasi cubic, nearest, dan linear secara manual
# def interpolate_manual(method, grid_x, grid_y, points, values):
#     if method == "cubic":
#         # Menggunakan interp2d untuk cubic interpolation
#         interpolator = interp2d(points[:, 0], points[:, 1], values, kind='cubic')
#         return interpolator(grid_x, grid_y)
#     elif method == "linear":
#         # Menggunakan interp2d untuk linear interpolation
#         interpolator = interp2d(points[:, 0], points[:, 1], values, kind='linear')
#         return interpolator(grid_x, grid_y)
#     elif method == "nearest":
#         # Implementasi nearest neighbor interpolation
#         grid_z = np.zeros(grid_x.shape)
#         for i in range(grid_x.shape[0]):
#             for j in range(grid_x.shape[1]):
#                 # Menemukan titik terdekat untuk setiap grid
#                 distances = np.sqrt((points[:, 0] - grid_x[i, j]) ** 2 + (points[:, 1] - grid_y[i, j]) ** 2)
#                 nearest_point_idx = np.argmin(distances)
#                 grid_z[i, j] = values[nearest_point_idx]
#         return grid_z


# Fungsi untuk interpolasi manual dengan griddata (cubic, linear, nearest)
def interpolate_manual(method, grid_x, grid_y, points, values):
...
selected2 = option_menu(
    ...
    ["Home", "Upload", "Graphics", "Map"],  # Opsi menu
    icons=["house", "cloud-upload", "search", "map"],  # Ikon yang sesuai
...
# ===============================
    ...
    # if st.sidebar.checkbox("Regional and Residual Anomaly Analysis"):
    #
    #     # Tambahan: Regional-Residual Separation
    #     st.subheader("Regional and Residual Anomaly Analysis")
    #
    #     # Pilih derajat polinomial
    #     poly_deg = st.sidebar.selectbox("Polynomial Degree (Regional Trend)", [1, 2, 3], index=1)
    #
    #      # Buat meshgrid sebagai X dan Y
    #     X = df_filtered['Longitude'].values
    #     Y = df_filtered['Latitude'].values
    #     Z = df_filtered["Bouguer Anomaly"].values
    #
    #
    # # Siapkan matriks polinomial
    ...
    #
    #
    #     A = poly_terms(X, Y, poly_deg)
    #     coeffs, _, _, _ = np.linalg.lstsq(A, Z, rcond=None)
    #     Z_regional = A @ coeffs
    #     Z_residual = Z - Z_regional
    #
    #     # Simpan ke dataframe
    #     df_filtered["FAA Regional"] = Z_regional
    #     df_filtered["FAA Residual"] = Z_residual
    #
    #     # Plot regional dan residual
    #     fig2, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))
    #     sc1 = ax1.tricontourf(X, Y, Z_regional, cmap=colormap, levels=20)
    #     fig2.colorbar(sc1, ax=ax1)
    #     ax1.set_title("Regional Anomaly (Polynomial deg={})".format(poly_deg))
    #     ax1.set_xlabel("Longitude")
    #     ax1.set_ylabel("Latitude")
    #
    #     sc2 = ax2.tricontourf(X, Y, Z_residual, cmap=colormap, levels=20)
